...
DATASET_SIZES = [10_000, 50_000, 100_000, 200_000]  # You can adjust as needed
...
def read_data() -> Dict[str, Any]:
    url = f"{BASE_URL}/polars-read/{SCHEMA_NAME}"
    process = psutil.Process(os.getpid())
    mem_start = process.memory_info().rss / (1024 * 1024)
    cpu_start = process.cpu_percent(interval=None)
    start = time.perf_counter()
...
def print_results_table(results: List[Dict[str, Any]]):
    headers = ["Dataset Size", "Operation", "Duration (s)", "Records", "CPU %", "Memory (MB)", "Status"]
    rows = []
...
def save_results_to_csv(results: List[Dict[str, Any]], filename: str = None):
    ...
    fieldnames = ["dataset_size", "operation", "duration_s", "records", "cpu_usage", "memory_usage_mb", "status"]
    try:
